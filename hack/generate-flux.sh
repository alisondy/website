...
generate() {
    # For commands in YAML file
    yq eval -o=p '.generate-cli | keys' gen-cli.yaml | cut -d ' ' -f3 | while read item; do
       
        # SNIPPET_PATH grabs the base path of where the snippet will go
        SNIPPET_PATH=$(yq eval ".generate-cli.$item.path"  gen-cli.yaml)

        # SNIPPET_FLAGS grabs the flags for the Flux command - without the export
        SNIPPET_FLAGS=$(yq eval ".generate-cli.$item.cli-flags"  gen-cli.yaml)

        OUT_SNIPPET_PATH=$SNIPPET_PATH$item.yaml
        IN_SNIPPET_PATH=$SNIPPET_PATH$item.sh
        ...
        "${TMP_DIR}/flux" $SNIPPET_FLAGS --export > "$OUT_SNIPPET_PATH"
...
# extract it to this directory, and this directory only

# flux-cli-gen.yaml
# 
# files:
#   'generate-source.sh':
#     cli-flags: 'create git'
#     path: 'snippet/docs/sources'
