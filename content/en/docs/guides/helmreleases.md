...
You need a Kubernetes cluster with the GitOps toolkit controllers installed on it.
If you do not already have the toolkit controllers setup, you can bootstrap them
by following the [get started guide](../get-started/index.md) or the [installation guide](../installation/) .

## Objectives

## Define a chart source

The source for the chart needs to be known to the source controller before a Helm chart can be released.

This source can be a `HelmRepository`, `GitRepository`, or `Bucket`
...
You can release Charts from Git repositories by declaring a
...
{{% alert color="info" title="Producing New Chart Artifacts" %}}
To make the source controller produce a new chart artifact, the `version` in the
...
#### Direct bucket source

You can use a `Bucket` as a source for a `HelmRelease`.

This is not recommended as the whole storage bucket is downloaded by source controller at each sync. The
bucket can become very large if there are frequent releases of multiple charts stored in the same bucket.

#### In Cluster Helm Repository

A better alternative to using a `bucket` is running an in cluster local helm repository, that refers to your cloud storage.

This cluster local Helm repository can then be used by source controller.

##### Example : Chartmuseum

You can create a in cluster helm repository with [Chartmuseum](https://github.com/helm/chartmuseum).
...
<!-- Add infromation here on checking chart museum is up and running-->

After Chartmuseum is running it should be possible to use the accompanying
...
You can use Kustomize [ConfigMap generator](https://kubectl.docs.kubernetes.io/references/kustomize/configmapgenerator/)
to trigger a Helm release upgrade every time the encoded values change.

### Create a `kustomizeconfig.yaml` for Kustomize to patch `ConfigMaps` referenced in `HelmRelease` manifests
...
<!-- This should be shifted into the notifications section, and this section should reference the page -->
...
first. See the [notifications guide](notifications.md#define-a-provider) for information on setting up a `Provider`.

#### Create a new `Alert` resource

Create an alert in the `flux-system` to start receiving notifications about the Helm
release:
...
To notify source controller about a chart update,
